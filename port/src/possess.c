 ...
 *
 *   - Solo / missions only (refused in Combat Sim).
 *   - Free-fly (no gravity), 6-DOF noclip movement driven by the gamepad/KBM,
 *     modelled directly on the spectator freecam (Kai's port/src/spectator.c).
 ...
 *
 * Backs pd.possess_spawn() / pd.unpossess() (see luaai_api_chrs.c). The per-frame
 * driver luaPossessReadInput() + camera apply luaPossessApplyCamera() are called
 * from the port frame loop (pdmain.c).
 *
 * This is the most experimental toolkit feature; everything here is port-only.
 *
 * From the Perfect Dark Kai fork (be46717), port/src/possess.c. This build has
 * no netplay, so the net-client refusal is gone; the Combat Sim refusal stays.
 */
...
// these match Kai spectator.c's SPEC_VK_* values).
...
// bridge (luaai_bridge_chrs.c) before this is reached, but we re-check defensively.
...
// Stage teardown (lvReset): the cube prop is gone with the stage, so drop the
// fly state without touching the camera (the players are being rebuilt).
// Not in Kai: there a stage change mid-possession kept overriding the camera.
void luaPossessStageReset(void)
{
	if (g_Possess.active) {
		g_PlayersWithControl[0] = g_Possess.savedctrl;
	}
...
// pdmain.c (before the per-player ticks). No-op unless possession is active.
...
void luaPossessApplyCamera(void)
{
	...
	// above is already recorded for lvRender to consume. Mirrors Kai's spectator.c
