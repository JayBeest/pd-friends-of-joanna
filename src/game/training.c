	...
	if (bgunGetWeaponNum(HAND_RIGHT) == WEAPON_FARSIGHT) {
		for (i = 0; i < ARRAYCOUNT(g_FrData.targets); i++) {
			if (g_FrData.targets[i].inuse
				...
				struct prop *prop = g_FrData.targets[i].prop;
				f32 xdiff = g_Vars.currentplayer->bond2.eyepos.x - prop->pos.x;
				f32 ydiff = g_Vars.currentplayer->bond2.eyepos.y - prop->pos.y;
				f32 zdiff = g_Vars.currentplayer->bond2.eyepos.z - prop->pos.z;
...
#else
		// >>> chrbios: generated from src/game/chrbios.md by tools/mkchrbios
