...
<#
copy-n64rom-to-pcbasedir
#>
task copy-n64rom-to-pcbasedir -action {
  pushd $env:PD
    cp build/ntsc-final/pd.z64 $script:PDDEST/pd.ntsc-final.z64 || write-error "Failed to copy pd.z64"
  popd
}
...
task make-moddir -action {

  $env:task_make_moddir = "fail"
  /app/python/mk-moddir || write-error "mk-moddir-failed"
  $env:task_make_moddir = "pass"

}

task install-foj -depends "make-moddir" -action {
  $env:task_install_foj = "fail"
  pushd $env:PD
    rsync -va --delete build/$env:ROMID/mod/ $env:PD_MODDIR/mod_fojo || write-error "Failed to copy mod files"
  popd
  $env:task_install_foj = "pass"
}
...
task foj  -depends "make-perfectdark", "make-moddir","install-foj"
